...
class SampleListModel(QAbstractListModel):
    ...
    def __init__(self,
        ...
        self.next_index = len(self.samples)
        self.current_sample = 0
    
    def add(self, sample):
        self.samples[next_index] = sample
        self.next_index += 1
        
    def add_samples(self, sample_ids):
        [self.samples[i] = {"sample_id": sample_id} for i, sample_id in enumerate(sample_ids)]
    
    def clear(self):
        [self.removeRow[i] for i in range(self.next_index-1)
        
    def data(self, index, role):
        if role == Qt.DisplayRow:
...
class ManageVialsWidget(QWidget):
    ...
    def __init__(self, model = None):
        ...
        self.sample_arrow_prev = QPushButton('<- Previous')
        self.sample_arrow_next = QPushButton('Next ->')
        self.sample_arrow_prev.setEnabled(False)
        self.sample_arrow_next.setEnabled(False)
        
        sample_arrows_layout = QHBoxLayout()
        sample_arrows_layout.addWidget(self.sample_arrow_prev)
        sample_arrows_layout.addWidget(self.sample_arrow_next)
        ...
        self.sample_view = SampleListView(model = self.sample_model)
            
        self.list_controls = VialListControlWidget()

        self.sample_view.currentItemChanged.connect(self.index_changed)
        ...
        self.sample_arrow_prev.clicked.connect(self.prev_vial)
        self.sample_arrow_next.clicked.connect(self.next_vial)

        self.layout.addWidget(self.list_controls)
        self.layout.addWidget(self.sample_view, 80)
        # self.layout.addWidget(self.nav_buttons)
        self.layout.addLayout(sample_arrows_layout)
    ...
    def index_changed(self, index):
        ...
        self.vialSelected.emit(str(index))
    ...
    def add_vial_unique(self, user_input):
        ...
        if user_input not in vials:
            ...
            self.sample_model.addItem(user_input)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, camera, *args, **kwargs):
        ...
        self.samples_model = SampleListModel()
        self.manage_vials_widget = ManageVialsWidget(self.samples_model)
        self.samples_view = self.manage_vials_widget.sample_view
        self.samples_view.setModel(self.samples_model)
        ...
        self.current_sample_id = None
        self.current_picpath = self.samples_model
        self.current_picpath.filepathChanged.connect(self.update_filename)
    ...
    def add_sample(self):
        self.samples_model.samples[self.samples_model.rowcount-1] = {"sample_id", "vial_id"}
        self.samples_model.layoutChanged.emit()
